...
class CrossEntropyLoss2d(nn.Module):
    ...
    def forward(self, *inputs):
        ...
        loss1 = F.binary_cross_entropy(F.sigmoid(pred1).squeeze(1), target)
        loss2 = F.binary_cross_entropy(F.sigmoid(pred2).squeeze(1), target)
        loss3 = F.binary_cross_entropy(F.sigmoid(pred3).squeeze(1), target)
        loss4 = F.binary_cross_entropy(F.sigmoid(pred4).squeeze(1), target)
...
def val(args, val_loader, model, criterion):
    ...
    for iter, (input, target) in enumerate(val_loader):
        ...
        if args.gpu and torch.cuda.device_count() > 1:
            ...
            output = output[0]
        iou_eval_val.add_batch((output.squeeze(1) > 0.5).data.cpu().numpy(), target_var.data.cpu().numpy())
...
def train(args, train_loader, model, criterion, optimizer, epoch, max_batches, cur_iter=0):
    ...
    for iter, (input, target) in enumerate(train_loader):
        ...
        if args.gpu and torch.cuda.device_count() > 1:
            ...
            output = output[0]
        iou_eval_train.add_batch((output.squeeze(1) > 0.5).data.cpu().numpy(), target_var.data.cpu().numpy())
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_name', default='InfNet', help='Model name')

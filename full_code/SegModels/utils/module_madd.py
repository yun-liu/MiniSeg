...
def compute_Bilinear_madd(module, inp, out):
    assert isinstance(module, nn.Bilinear)
    assert len(inp[0].size()) == 2 and len(out.size()) == 2
    
    num_in_features_1 = inp[0].size()[1]
    num_in_features_2 = inp[1].size()[1]
    num_out_features = out.size()[1]
...
def compute_module_madd(module, inp, out):
    if isinstance(module, nn.Conv2d):
        return compute_Conv2d_madd(module, inp[0], out)
    elif isinstance(module, nn.ConvTranspose2d):
        return compute_ConvTranspose2d_madd(module,  inp[0], out)
    elif isinstance(module, nn.BatchNorm2d):
        return compute_BatchNorm2d_madd(module,  inp[0], out)
    elif isinstance(module, nn.MaxPool2d):
        return compute_MaxPool2d_madd(module,  inp[0], out)
    elif isinstance(module, nn.AvgPool2d):
        return compute_AvgPool2d_madd(module,  inp[0], out)
    elif isinstance(module, (nn.ReLU, nn.ReLU6)):
        return compute_ReLU_madd(module,  inp[0], out)
    elif isinstance(module, nn.Softmax):
        return compute_Softmax_madd(module,  inp[0], out)
    elif isinstance(module, nn.Linear):
        return compute_Linear_madd(module,  inp[0], out)
    elif isinstance(module, nn.Bilinear):
        return compute_Bilinear_madd(module, inp, out)
    else:
        return 0

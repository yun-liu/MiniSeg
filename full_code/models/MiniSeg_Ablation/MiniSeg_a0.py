...
class DilatedParallelConvBlock(nn.Module):
    def __init__(self, in_planes, out_planes, stride=1, groups=4):
        ...
        self.conv3x3 = nn.Conv2d(inter_planes, inter_planes, 3, stride=stride, padding=1, dilation=1, groups=inter_planes, bias=False)
        self.conv1x1_fuse = nn.Conv2d(inter_planes, out_planes, 1, padding=0, groups=groups, bias=False)
        self.bn = nn.BatchNorm2d(out_planes)
        self.act = nn.PReLU(out_planes)
    ...
    def forward(self, input):
        ...
        output = self.conv1x1_fuse(output)
...
class MiniSeg(nn.Module):
    def __init__(self, classes=2, P1=2, P2=3, P3=8, P4=6, aux=True):
        ...
        self.down2 = DilatedParallelConvBlock(8, 24, stride=2, groups=2)
        self.level2 = nn.ModuleList()
        for i in range(0, P2):
            self.level2.append(DilatedParallelConvBlock(24, 24, groups=2))

        self.down3 = DilatedParallelConvBlock(24, 32, stride=2, groups=2)

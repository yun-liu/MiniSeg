...
class DilatedParallelConvBlock(nn.Module):
    def __init__(self, in_planes, out_planes, stride=1):
        ...
        self.conv5 = nn.Conv2d(inter_planes, inter_planes, 3, stride=stride, padding=8, dilation=8, groups=inter_planes, bias=False)
        self.conv6 = nn.Conv2d(inter_planes, inter_planes, 3, stride=stride, padding=8, dilation=8, groups=inter_planes, bias=False)
        self.conv7 = nn.Conv2d(inter_planes, inter_planes, 3, stride=stride, padding=8, dilation=8, groups=inter_planes, bias=False)
        self.conv8 = nn.Conv2d(inter_planes, inter_planes, 3, stride=stride, padding=8, dilation=8, groups=inter_planes, bias=False)
        self.pool = nn.AvgPool2d(3, stride=stride, padding=1)
        self.conv1x1_fuse = nn.Conv2d(out_planes, out_planes, 1, padding=0, groups=8, bias=False)
        self.attention = nn.Conv2d(out_planes, 8, 1, padding=0, groups=8, bias=False)
        self.bn = nn.BatchNorm2d(out_planes)
        self.act = nn.PReLU(out_planes)
    ...
    def forward(self, input):
        ...
        d7 = self.conv7(output)
        d8 = self.conv8(output)
        p = self.pool(output)
        d1 = d1 + p
        d2 = d1 + d2
        d3 = d2 + d3
        ...
        d7 = d6 + d7
        d8 = d7 + d8
        att = torch.sigmoid(self.attention(torch.cat([d1, d2, d3, d4, d5, d6, d7, d8], 1)))
        d1 = d1 + d1 * att[:, 0].unsqueeze(1)
        d2 = d2 + d2 * att[:, 1].unsqueeze(1)
        d3 = d3 + d3 * att[:, 2].unsqueeze(1)
        d4 = d4 + d4 * att[:, 3].unsqueeze(1)
        d5 = d5 + d5 * att[:, 3].unsqueeze(1)
        d6 = d6 + d6 * att[:, 3].unsqueeze(1)
        d7 = d7 + d7 * att[:, 3].unsqueeze(1)
        d8 = d8 + d8 * att[:, 3].unsqueeze(1)
        output = self.conv1x1_fuse(torch.cat([d1, d2, d3, d4, d5, d6, d7, d8], 1))

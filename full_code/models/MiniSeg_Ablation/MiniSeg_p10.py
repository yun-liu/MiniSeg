...
class MiniSeg(nn.Module):
    def __init__(self, classes=2, P1=2, P2=3, P3=8, P4=6, aux=True):
        ...
        self.cat2 = nn.Sequential(
                    nn.Conv2d(96, 96, 1, stride=1, padding=0, groups=1, bias=False),
                    nn.BatchNorm2d(96))
        ...
        self.long4 = DownsamplerBlock(64, 192, stride=2)
        self.down4 = DilatedParallelConvBlock(64, 192, stride=2)
        self.level4 = nn.ModuleList()
        self.level4_long = nn.ModuleList()
        for i in range(0, P4):
            self.level4.append(DilatedParallelConvBlock(192, 192))
        for i in range(0, self.D4):
            self.level4_long.append(DownsamplerBlock(192, 192, stride=1))

        self.up4_conv4 = nn.Conv2d(192, 192, 1, stride=1, padding=0)
        self.up4_bn4 = nn.BatchNorm2d(192)
        self.up4_act = nn.PReLU(192)

        self.up3_conv4 = DilatedParallelConvBlockD2(192, 64)
        ...
        if self.aux:
            self.pred4 = nn.Sequential(nn.Dropout2d(0.01, False), nn.Conv2d(192, classes, 1, stride=1, padding=0))

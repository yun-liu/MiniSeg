...
class MiniSeg(nn.Module):
    def __init__(self, classes=2, P1=2, P2=3, P3=8, P4=6, aux=True):
        ...
        self.long2 = DownsamplerBlock(16, 32, stride=2)
        self.down2 = DilatedParallelConvBlock(16, 32, stride=2)
        self.level2 = nn.ModuleList()
        self.level2_long = nn.ModuleList()
        for i in range(0, P2):
            self.level2.append(DilatedParallelConvBlock(32, 32))
        for i in range(0, self.D2):
            self.level2_long.append(DownsamplerBlock(32, 32, stride=1))

        self.cat2 = nn.Sequential(
                    nn.Conv2d(64, 64, 1, stride=1, padding=0, groups=1, bias=False),
                    nn.BatchNorm2d(64))

        self.long3 = DownsamplerBlock(32, 64, stride=2)
        self.down3 = DilatedParallelConvBlock(32, 64, stride=2)
        ...
        self.up2_conv2 = nn.Conv2d(32, 32, 1, stride=1, padding=0)
        ...
        if self.aux:
            ...
            self.pred2 = nn.Sequential(nn.Dropout2d(0.01, False), nn.Conv2d(32, classes, 1, stride=1, padding=0))

...
class MiniSeg(nn.Module):
    def __init__(self, classes=2, P1=2, P2=3, P3=8, P4=6, aux=True):
        ...
        self.long2 = DownsamplerBlock(8, 12, stride=2)
        self.down2 = DilatedParallelConvBlock(8, 12, stride=2)
        self.level2 = nn.ModuleList()
        self.level2_long = nn.ModuleList()
        for i in range(0, P2):
            self.level2.append(DilatedParallelConvBlock(12, 12))
        for i in range(0, self.D2):
            self.level2_long.append(DownsamplerBlock(12, 12, stride=1))

        self.cat2 = nn.Sequential(
                    nn.Conv2d(24, 24, 1, stride=1, padding=0, groups=1, bias=False),
                    nn.BatchNorm2d(24))

        self.long3 = DownsamplerBlock(12, 24, stride=2)
        self.down3 = DilatedParallelConvBlock(12, 24, stride=2)
        self.level3 = nn.ModuleList()
        self.level3_long = nn.ModuleList()
        for i in range(0, P3):
            self.level3.append(DilatedParallelConvBlock(24, 24))
        for i in range(0, self.D3):
            self.level3_long.append(DownsamplerBlock(24, 24, stride=1))

        self.cat3 = nn.Sequential(
                    nn.Conv2d(48, 48, 1, stride=1, padding=0, groups=1, bias=False),
                    nn.BatchNorm2d(48))

        self.long4 = DownsamplerBlock(24, 48, stride=2)
        self.down4 = DilatedParallelConvBlock(24, 48, stride=2)
        self.level4 = nn.ModuleList()
        self.level4_long = nn.ModuleList()
        for i in range(0, P4):
            self.level4.append(DilatedParallelConvBlock(48, 48))
        for i in range(0, self.D4):
            self.level4_long.append(DownsamplerBlock(48, 48, stride=1))

        self.up4_conv4 = nn.Conv2d(48, 48, 1, stride=1, padding=0)
        self.up4_bn4 = nn.BatchNorm2d(48)
        self.up4_act = nn.PReLU(48)

        self.up3_conv4 = DilatedParallelConvBlockD2(48, 24)
        self.up3_conv3 = nn.Conv2d(24, 24, 1, stride=1, padding=0)
        self.up3_bn3 = nn.BatchNorm2d(24)
        self.up3_act = nn.PReLU(24)

        self.up2_conv3 = DilatedParallelConvBlockD2(24, 12)
        self.up2_conv2 = nn.Conv2d(12, 12, 1, stride=1, padding=0)
        self.up2_bn2 = nn.BatchNorm2d(12)
        self.up2_act = nn.PReLU(12)

        self.up1_conv2 = DilatedParallelConvBlockD2(12, 8)
        self.up1_conv1 = nn.Conv2d(8, 8, 1, stride=1, padding=0)
        self.up1_bn1 = nn.BatchNorm2d(8)
        self.up1_act = nn.PReLU(8)

        if self.aux:
            self.pred4 = nn.Sequential(nn.Dropout2d(0.01, False), nn.Conv2d(48, classes, 1, stride=1, padding=0))
            self.pred3 = nn.Sequential(nn.Dropout2d(0.01, False), nn.Conv2d(24, classes, 1, stride=1, padding=0))
            self.pred2 = nn.Sequential(nn.Dropout2d(0.01, False), nn.Conv2d(12, classes, 1, stride=1, padding=0))

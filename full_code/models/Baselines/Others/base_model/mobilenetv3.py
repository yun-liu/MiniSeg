...
from models.baselines.base_model.basic import _Hswish, _ConvBNHswish, Bottleneck, SEModule

__all__ = ['MobileNetV3', 'get_mobilenet_v3', 'mobilenet_v3_large_1_0', 'mobilenet_v3_small_1_0']


class MobileNetV3(nn.Module):
    def __init__(self, nclass=2, mode='large', width_mult=1.0, dilated=False, norm_layer=nn.BatchNorm2d):
...
def get_mobilenet_v3(mode='small', width_mult=1.0, pretrained=False, root='~/,torch/models'):
    model = MobileNetV3(mode=mode, width_mult=width_mult)
    if pretrained:
        raise ValueError("Not support pretrained")
    return model


def mobilenet_v3_large_1_0():
    return get_mobilenet_v3('large', 1.0)


def mobilenet_v3_small_1_0():
    return get_mobilenet_v3('small', 1.0)

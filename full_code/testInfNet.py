...
def validate(args, model, image_list, label_list, crossVal, mean, std):
    ...
    for idx in range(len(image_list)):
        ...
        class_numpy = img_out[-1].sigmoid_().squeeze().data.cpu().numpy()
...
def main_te(args, crossVal, pretrained, mean, std):
    ...
    model = net.InfNet(args.classes, aux=False)
...
def main(args):
    ...
    for i in range(crossVal):
        ...
        mean = np.array([0.485, 0.456, 0.406], dtype=np.float32)#data['mean']
        std = np.array([0.229, 0.224, 0.225], dtype=np.float32)#data['std']
        ...
        main_te(args, i, pretrainedModel, mean, std)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--classes', default=1, type=int, help='Number of classes in the dataset')
